...
import { checkBlogPostSchema, checkValidationResult } from "../authors/validator.js"
...
blogsRouter.post("/:id/comments", checkBlogPostSchema, checkValidationResult, async (req, res, next) => {
    try {
        const errorsList = validationResult(req)
        if (!errorsList.isEmpty()) {
...
blogsRouter.post("/:id/uploadCover", checkBlogPostSchema, checkValidationResult, async (req, res, next) => {
    try {
        const errorsList = validationResult(req)
        if (!errorsList.isEmpty()) {
            next(createHttpError(400, { errorsList }))
